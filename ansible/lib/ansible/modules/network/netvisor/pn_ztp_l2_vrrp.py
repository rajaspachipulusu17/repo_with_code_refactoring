...
def configure_vrrp(module, csv_data, CHANGED_FLAG, task, msg):
    ...
    for row in csv_data_list:
        if not row or row.startswith('#'):
            ...
            spine_switch_1 = elements.pop(0).strip()
            if module.params['pn_current_switch'] == spine_switch_1:
                if len(elements) > 2:
                    spine_switch_2 = elements.pop(0).strip()
                    vrrp_id = elements.pop(0).strip()
                    active_switch = elements.pop(0).strip()
                    switch_list.append(spine_switch_1)
                    switch_list.append(spine_switch_2)
                    mod = 'l2-vrrp'
                    ...
                    result, CHANGED_FLAG = configure_vrrp_for_non_clustered_switches(
                            module, vlan_id, vrrp_ip, vrrp_ipv6, spine_switch_1,
