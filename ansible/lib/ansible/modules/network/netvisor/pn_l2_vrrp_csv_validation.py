...
description:
    L2 VRRP config csv file format:
...
def main():
    ...
    vlan_list = []
    existing_ip = []
    csv_data = module.params['pn_csv_data'].splitlines()
    switch_id_dict = {}
            ...
            if not row or row.startswith('#'):
                ...
                if len(elements) != 3 and len(elements) != 6:
                    ...
                    if not ip or not vlan or not switch1:
                        ...
                        if not validate_switch_name(switch1, spine_list):
                            output += 'Invalid switch name {} '.format(switch1)
                            output += 'at line number {}\n'.format(line_count)
                        else:
                            clustered_spines.append(switch1)
                    ...
                    if len(elements) == 6:
                        ...
                        if not switch2 or not vrrp_id or not active_switch:
                            ...
                            if not validate_switch_name(switch2, spine_list):
                                output += 'Invalid switch name {} '.format(
                                    switch2)
                                output += 'at line number {}\n'.format(
                                    line_count)
                            else:
                                clustered_spines.append(switch2)
                                if tuple(clustered_spines) not in switch_id_dict:
                                    switch_id_dict[
                                        tuple(clustered_spines)] = None
                            ...
                            if not vrrp_id.isdigit():
                                ...
                                existing_vrrp_id = switch_id_dict.get(
                                    tuple(clustered_spines))
                                if existing_vrrp_id is None:
                                    switch_id_dict[
                                        tuple(clustered_spines)] = vrrp_id
                                else:
                            ...
                            if (not validate_switch_name(
                                    active_switch, spine_list) or
                                    active_switch not in clustered_spines):
                                output += 'Invalid switch name {} '.format(
                                    active_switch)
                                output += 'at line number {}\n'.format(
                                    line_count)
